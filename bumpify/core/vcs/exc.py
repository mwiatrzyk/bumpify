...
class VcsError(exc.BumpifyError):
    ...
    def __init__(self, root_dir: str, original_exc: Exception=None):
...
class RepositoryNotClean(VcsError):
    """Raised when clean repository is required, but instead uncommitted
    changes or untracked files are present."""
...
class TagAlreadyExists(VcsError):
    ...
    __message_template__ = "{self.tag_name} (in repository at: {self.path})"
...
class BranchAlreadyExists(VcsError):
    ...
    __message_template__ = "{self.branch_name} (in repository at: {self.path})"
